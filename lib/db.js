...
// Postgres caps a statement at 65535 bind parameters, but the Neon serverless
// (SQL-over-HTTP) endpoint rejects large request bodies well before that with a
// generic "Database request failed" — empirically a 17-col × 2941-row upsert
// (~50k params) fails while ~1460 rows succeeds. So we bound BOTH the parameter
// count and the row count per chunk; 1000 rows leaves comfortable margin for
// wide/jsonb/text-heavy rows.
...
export async function bulkUpsert(sql, { table, columns, rows, conflict, update, casts = {} }) {
  ...
  const chunkSize = Math.max(1, Math.min(Math.floor(MAX_PARAMS / columns.length), MAX_ROWS));
