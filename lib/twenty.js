...
/**
 * Write enrichment onto an existing contact: LinkedIn URL and/or mobile phone.
 * Only sets the provided fields. `phone` is an E.164 string; we also split the
 * "+<cc>" calling code so Twenty renders it nicely. Returns true if it patched.
 */
export async function updatePersonContact(personId, { linkedinUrl, phone } = {}) {
  if (!personId) return false;
  const body = {};
  if (linkedinUrl) body.linkedinLink = { primaryLinkUrl: linkedinUrl, primaryLinkLabel: 'LinkedIn' };
  // Store the full E.164 number; Twenty parses the country/calling code from it.
  if (phone) body.phones = { primaryPhoneNumber: phone };
  if (!Object.keys(body).length) return false;
  await twFetch(`/people/${personId}`, { method: 'PATCH', body });
  return true;
}
