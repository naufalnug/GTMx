export const clients = {
  storylane: {
    name: 'Storylane.io',
    // EmailBison workspace 3 on send.gtmx.run. Reached with a dedicated client
    // key (not an adminWorkspaceId switch). clients.workspace_id is portal-admin
    // metadata and is not read by the sync — do not confuse it with MDJ's 7.
    workspaceId: 3,
  ...
  uksalesincrease: {
    ...
    workspaceId: 11,
    // Same shape as lookmedia and flexetc: a child workspace on send.gtmx.run with
    // no dedicated client key, reached with the shared admin key.
    adminWorkspaceId: 11,
    instanceUrlEnvVar: 'EMAILBISON_GTMX_INSTANCE_URL',
    apiKeyEnvVar: 'EMAILBISON_ADMIN_API_KEY',
