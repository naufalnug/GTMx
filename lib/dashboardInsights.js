export function deriveInsights(campaigns) {
  ...
  if (highestVolume && highestVolume.replies > 0 && highestVolume.id !== best?.id) {
    insights.push({
      kind: 'volume',
      label: 'Most replies',
  ...
  const skipIds = new Set([best?.id, highestVolume?.id, watch?.id].filter(Boolean));
