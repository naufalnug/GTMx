...
async function supabaseUpsert(client, table, rows, conflict) {
  ...
  const unique = dedupeByConflict(rows, conflict);
  if (unique.length < rows.length) {
    console.warn(
      `${table}: dropped ${rows.length - unique.length} duplicate (${conflict.join(',')}) row(s) before upsert`
    );
  }
  for (let index = 0; index < unique.length; index += 500) {
    const { error } = await client.from(table).upsert(unique.slice(index, index + 500), { onConflict: conflict.join(',') });
...
export async function syncClient(cfg, opts = {}) {
  ...
  // Resume from a saved cursor whether the timed-out run was a full backfill or an
  // incremental one. Gating this on `!lastSynced` meant an incremental walk that ran
  // out of time saved a cursor nobody ever read: last_synced_at never advanced, so
  // the next run recomputed the same oversized window and timed out in the same
  // place. gtmx was stuck in exactly that loop. The cursor stays valid because the
  // window is derived from last_synced_at, which does not move until a walk finishes.
  const resumeCursor = clientRow?.contacts_cursor ?? null;
