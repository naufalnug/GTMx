...
export async function syncClient(cfg, { log = () => {} } = {}) {
  ...
  await upsertChunks(sb, 'campaigns', campaignRows, 'client,source,campaign_id');
  await upsertChunks(sb, 'campaign_daily_stats', dailyRows, 'client,source,campaign_id,date');
  await upsertChunks(sb, 'sequence_steps', stepRows, 'client,source,campaign_id,step_id');
  await upsertChunks(sb, 'interested_replies', replyRows, 'client,source,reply_id');
  log(
  ...
  for await (const batch of iterateContacts(baseUrl, apiKey)) {
    ...
    await upsertChunks(sb, 'contacts', rows, 'client,source,lead_id');
    contactCount += rows.length;
    if (contactCount % 2000 < rows.length) log(`${slug}: ${contactCount} contacts…`);
  }

  await sb.from('clients').update({ last_synced_at: new Date().toISOString() }).eq('slug', slug);
