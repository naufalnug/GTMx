...
async function* iterateContacts(base, key, { startCursor = null, onCursor = () => {} } = {}) {
  const firstQs = `${LEADS_FILTER}&pagination_type=cursor${
    startCursor ? `&cursor=${encodeURIComponent(startCursor)}` : ''
  }`;
  let probe = await ebFetch(base, key, `/api/leads?${firstQs}`);
  // A stored cursor can expire. Empty first page → restart from the beginning
  // rather than treat the pull as complete (which would skip remaining leads).
  if (startCursor && !(probe?.data ?? []).length) {
    startCursor = null;
    probe = await ebFetch(base, key, `/api/leads?${LEADS_FILTER}&pagination_type=cursor`);
  }
  ...
  if (useCursor) {
    ...
    for (let i = 0; i < LEADS_MAX_PAGES; i++) {
      ...
      const next = batch.length ? res?.meta?.next_cursor ?? null : null;
      onCursor(next);
      ...
      res = await ebFetch(
        base,
        key,
        `/api/leads?${LEADS_FILTER}&pagination_type=cursor&cursor=${encodeURIComponent(next)}`
      );
    }
    return;
  }
...
async function stampClient(db, slug, fields) {
...
async function loadInterestedEmails(db, slug) {
  ...
  for (let from = 0; ; from += pageSize) {
    ...
    if (error) throw new Error(`interested_replies read failed: ${error.message}`);
    for (const row of data ?? []) {
      const email = (row.email || '').toLowerCase();
      if (email) emails.add(email);
    }
    if (!data || data.length < pageSize) return emails;
  }
}
 ...
 *
 * Campaign stats stamp `last_synced_at` as soon as they succeed, so a later
 * contacts timeout cannot leave the dashboard badge stuck. A contacts failure
 * is recorded on `sync_error` (and a resume cursor, when the instance supports
 * it) so the next invocation can skip stats and finish the leads pull.
 *
 * `opts.hardStop` (epoch ms) aborts the contacts loop in time to write that
 * error before the serverless ceiling kills the process. `opts.stage` selects
 * stats, contacts, or both (see resolveSyncStages).
 */
export async function syncClient(cfg, { log = () => {}, hardStop, stage } = {}) {
  const { slug, baseUrl, apiKey } = cfg;
  const db = createAdminClient();
  const source = sourceFromUrl(baseUrl);

  const { data: state } = await db.from('clients').select('sync_error').eq('slug', slug).maybeSingle();
  const { runStats, runContacts } = resolveSyncStages({ stage, syncError: state?.sync_error });
  const resumeCursor = runStats ? null : parseContactsResumeCursor(state?.sync_error);
  log(`${slug}: stages stats=${runStats} contacts=${runContacts}${resumeCursor ? ' (resuming contacts)' : ''}`);
  ...
  if (runStats) {
    ...
    // Stamp now, before the contacts pull. A Vercel kill mid-contacts used to
    // leave last_synced_at untouched (and sync_error null, because catch never
    // ran). The dashboard reads this stamp; contacts failures go on sync_error.
    await stampClient(db, slug, {
      last_synced_at: new Date().toISOString(),
      sync_error: runContacts ? contactsIncompleteMessage({ detail: 'pull in progress' }) : null,
    });
  }
  ...
  if (!runContacts) {
    log(`${slug}: contacts skipped (stage=stats)`);
    return { ...statsResult, contacts: 0, contactsSkipped: true };
  }

  if (pastDeadline(hardStop)) {
    const syncError = contactsIncompleteMessage({
      detail: 'ran out of time before contacts pull',
      cursor: resumeCursor,
    });
    await stampClient(db, slug, { sync_error: syncError });
    log(`${slug}: ${syncError.split('\n')[0]}`);
    return { ...statsResult, contacts: 0, contactsIncomplete: true };
  }

  let contactCount = 0;
  let nextCursor = resumeCursor;
  try {
    const interestedEmails = runStats
      ? new Set(replyRows.map((r) => (r.email || '').toLowerCase()).filter(Boolean))
      : await loadInterestedEmails(db, slug);

    const seenLeadIds = [];
    const fullContactsPull = !resumeCursor;
    ...
    for await (const batch of iterateContacts(baseUrl, apiKey, {
      ...
      if (pastDeadline(hardStop)) {
        const syncError = contactsIncompleteMessage({ pulled: contactCount, cursor: nextCursor });
        await stampClient(db, slug, { sync_error: syncError });
        log(`${slug}: ${syncError.split('\n')[0]}`);
        return { ...statsResult, contacts: contactCount, contactsIncomplete: true };
      }
    }
    ...
    // A resumed pull only saw the tail of the list, so pruning from it would
    // delete every lead already stored from earlier pages.
    if (fullContactsPull && seenLeadIds.length > 0) {
    ...
    } else if (!fullContactsPull) {
      log(`${slug}: skipped prune — resumed contacts pull (not a full walk)`);
    }

    await stampClient(db, slug, { last_synced_at: new Date().toISOString(), sync_error: null });
    log(`${slug}: done — ${contactCount} contacts`);
    return { ...statsResult, contacts: contactCount };
  } catch (error) {
    // Stats already stamped last_synced_at. Keep that stamp, record the real
    // contacts error (with resume cursor) so a follow-up can skip stats.
    const syncError = contactsIncompleteMessage({
      pulled: contactCount,
      cursor: nextCursor,
    ...
    await stampClient(db, slug, { sync_error: syncError });
    log(`${slug}: contacts FAILED — ${error.message}`);
    return { ...statsResult, contacts: contactCount, contactsIncomplete: true, error: error.message };
  }
}
 ...
 * `opts.hardStop` is forwarded to each client so a contacts pull can abort and
 * write sync_error before the serverless runtime kills the process.
 */
