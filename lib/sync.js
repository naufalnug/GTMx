...
export async function syncClient(cfg, { log = () => {} } = {}) {
  ...
  if (seenLeadIds.length > 0) {
    const existing = (await sql.query(
      'select count(*)::int as c from contacts where client = $1 and source = $2',
      [slug, source]
    ))[0]?.c ?? 0;
    if (seenLeadIds.length >= Math.floor(existing * 0.5)) {
      const before = existing;
      await sql.query(
        'delete from contacts where client = $1 and source = $2 and lead_id <> all($3::bigint[])',
        [slug, source, seenLeadIds]
      );
      const after = (await sql.query(
        'select count(*)::int as c from contacts where client = $1 and source = $2',
        [slug, source]
      ))[0]?.c ?? 0;
      if (before - after > 0) log(`${slug}: pruned ${before - after} stale contacts (kept ${after})`);
    } else {
