...
async function* iterateContacts(base, key, since = null) {
...
export async function syncClient(cfg, { log = () => {} } = {}) {
  ...
  const [{ data: knownCampaigns }, { data: clientRow }] = await Promise.all([
    ...
    db.from('clients').select('last_synced_at').eq('slug', slug).maybeSingle(),
  ]);
  ...
  let contactCount = 0;
  const seenLeadIds = [];
  for await (const batch of iterateContacts(baseUrl, apiKey, lastSynced ? windowStart : null)) {
