...
export async function getPortalDashboard(client, period, source) {
  ...
  // Five daily series. Sends run ~1000/day while replies and bounces run in the tens,
  // so the chart facets these rather than sharing one axis — on a common scale the
  // small series are a 1px sliver.
  const trend = new Map();
  const point = (date) => {
    if (!trend.has(date)) trend.set(date, { date, sent: 0, replies: 0, bounced: 0, positive: 0, contacts: 0 });
    return trend.get(date);
  };
  for (const row of daily) {
    const day = point(row.date);
    day.sent += Number(row.sent ?? 0);
    day.replies += Number(row.replies ?? 0);
    day.bounced += Number(row.bounced ?? 0);
  }
  // Positive replies are dated by when the reply arrived, not by campaign day.
  for (const reply of replies) {
    if (!reply.date_received) continue;
    point(String(reply.date_received).slice(0, 10)).positive += 1;
  }
  // Contacts added, grouped in the database (see the contacts_daily view) rather than
  // by pulling every contact row.
  let contactsDaily = scoped(
    supabase.from('contacts_daily').select('date, contacts').eq('client', client), source
  );
  if (windowed) contactsDaily = contactsDaily.gte('date', period.start).lte('date', period.end);
  const { data: contactDays } = await contactsDaily;
  for (const row of contactDays ?? []) point(row.date).contacts += Number(row.contacts ?? 0);
