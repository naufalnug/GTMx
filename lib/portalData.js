...
export async function getPortalDashboard(client, period, source) {
  ...
  // Daily series behind the activity chart. The chart splits these into disjoint
  // buckets that sum to `sent`, so they must stay raw here.
  const trend = new Map();
  const point = (date) => {
    if (!trend.has(date)) trend.set(date, { date, sent: 0, replies: 0, bounced: 0, positive: 0 });
...
export async function getPortalNotifications(client, { userId, limit = 20 } = {}) {
  ...
  // Independent reads, so issue them together. Serialised, each costs a full round
  // trip to a database that may be a long way from the function.
  ...
  const watermark = Number(membership.data?.notifications_read_through_id ?? 0);
