...
// cron (app/api/cron/sync-crm). Idempotency is enforced by the Supabase
...
async function getLedgerRow(sb, workspaceId, replyId) {
...
async function writeLedger(sb, row) {
  const { error } = await sb
    .from(LEDGER_TABLE)
    .upsert({ ...row, updated_at: new Date().toISOString() }, { onConflict: 'workspace_id,reply_id' });
  if (error) throw new Error(`ledger write: ${error.message}`);
}
 ...
 */
export async function pushReplyToTwenty({ reply, lead }, { sb, cfg, log = () => {}, dryRun = false } = {}) {
  ...
  const existing = (await getLedgerRow(sb, workspaceId, replyId)) ?? {};
  ...
  try {
    ...
    if (!companyId) {
      ...
      if (companyId) await writeLedger(sb, { ...base, ...existing, crm_company_id: companyId });
    }
    ...
    if (!personId) {
      ...
      if (personId) await writeLedger(sb, { ...base, crm_company_id: companyId, crm_person_id: personId });
    }
    ...
    await writeLedger(sb, { ...base, ...existing, error: String(err?.message || err).slice(0, 500) }).catch(() => {});
 ...
 */
export async function reconcileInterestedReplies({ sb, log = () => {}, dryRun = false } = {}) {
  ...
  for await (const page of iterateInterestedReplies(cfg)) {
    for (const reply of page) {
      ...
      const res = await pushReplyToTwenty({ reply, lead }, { sb, cfg, log, dryRun });
