...
// Ensure the ledger has the opportunity-tracking column (added after the table
// was first created). Idempotent; runs once per process.
let _schemaReady = false;
async function ensureSchema(sql) {
  if (_schemaReady) return;
  await sql.query(`ALTER TABLE ${LEDGER_TABLE} ADD COLUMN IF NOT EXISTS crm_opportunity_id uuid`);
  _schemaReady = true;
}
...
export async function pushReplyToTwenty({ reply, lead }, { cfg, log = () => {}, dryRun = false } = {}) {
  ...
  try {
    ...
    // 3) Opportunity — a Lead-stage deal card linked to company + contact
    let opportunityId = existing.crm_opportunity_id ?? null;
    if (!opportunityId) {
      opportunityId = await upsertOpportunity({ name: fields.company, companyId, personId });
      if (opportunityId) {
    ...
    // 4) Note (write back id immediately — this is what marks the reply "done")
    ...
    // 5) Link note to person + company (self-dedupes)
    await linkNote(noteId, { personId, companyId });

    log(`reply ${replyId}: pushed (company=${companyId} person=${personId} opp=${opportunityId} note=${noteId})`);
    return { status: 'pushed', companyId, personId, opportunityId, noteId };
  } catch (err) {
