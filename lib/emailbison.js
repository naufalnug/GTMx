...
export function normalizeInterestedReply(reply, campaign) {
  ...
  return {
    ...
    email: lead.email ?? reply?.from_email_address ?? '',
...
export async function loadDashboardData(config) {
  ...
  const enriched = await Promise.all(
    campaigns.map(async (c) => {
      const [steps, interestedRaw] = await Promise.all([
      ...
      const interested = interestedRaw.length;
      return {
        id: c.id,
        name: c.name,
  ...
  for (const c of enriched) {
    for (const r of c._interestedRaw) {
      positiveReplies.push(normalizeInterestedReply(r, c));
    }
    delete c._interestedRaw;
  }
