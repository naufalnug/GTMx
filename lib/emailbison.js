...
// Without a timeout a throttled connection hangs forever, and any Promise.all waiting
// on it never settles — that is a silent, permanent sync stall rather than an error.
const FETCH_TIMEOUT_MS = 30_000;

async function ebFetch({ baseUrl, apiKey }, path, attempt = 0) {
  ...
  try {
    ...
    if (res.status === 429 && attempt < 2) {
      ...
      return ebFetch({ baseUrl, apiKey }, path, attempt + 1);
    }
    if (!res.ok) {
    ...
    // One retry covers a timeout or a dropped connection under load.
    if (attempt < 1) {
...
/**
 * Map with a concurrency cap. Firing every page of a busy campaign at once (some have
 * 50+) is what triggers the throttling this module then has to survive.
 */
...
export async function getInterestedReplies(config, campaignId) {
  ...
  let failed = 0;
  const rest = await mapLimit(pages, 6, (p) =>
    ebFetch(config, `/api/replies?campaign_id=${campaignId}&page=${p}`)
      .then((d) => (d?.data ?? []).filter((r) => r?.interested === true))
      .catch(() => {
        // A dropped page silently undercounts interested leads, so say so rather
        // than quietly reporting a smaller number than the client actually has.
        failed += 1;
        return [];
      })
  );
  if (failed) {
    console.error(
      `[emailbison] campaign ${campaignId}: ${failed}/${lastPage} reply pages failed — interested count may be low`
    );
  }
