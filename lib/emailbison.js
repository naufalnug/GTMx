...
export async function loadDashboardData(config) {
  ...
  const enriched = await Promise.all(
    campaigns.map(async (c) => {
      const [steps, interested] = await Promise.all([
        getSequenceSteps(config, c.id).catch(() => []),
        getInterestedCount(config, c.id).catch(() => Number(c.interested ?? 0)),
      ]);
  ...
  const totals = enriched.reduce(
    ...
    { sent: 0, replies: 0, bounced: 0, interested: 0, totalLeads: 0 }
  );
